...
{
    public class ParseMetadataV7 : IParseFactory<MetadataV7>
    {
        public MetadataV7 Parse(JObject json)
        {
            ...
            for (var moduleIndex = 0; moduleIndex < mLen.Value; moduleIndex++)
            {
                ...
                if (storageIsset != 0)
                {
                    module.Storage = new StorageCollectionV7();
                    var storageList = new List<StorageV7>();
                    ...
                    {
                        storageList.Add(new StorageV7());
                    }

                    for (int i = 0; i < storageLen.Value; i++)
                    {
                        storageList.Add(GetStorageV7(ref str));
                    }
                ...
                var constsList = new List<ConstV7>();
                for (int i = 0; i < constsCount.Value; i++)
                {
                     constsList.Add(GetConstV7(ref str));
                }
        ...
        private EventArgV7 GetEventV7(ref string str)
        {
            var ea = new EventArgV7();
        ...
        private CallV7 GetCallV7(ref string str)
        {
            var call = new CallV7();
            ...
            // args count
            var argList = new List<FunctionCallArgV7>();
            var args = Scale.DecodeCompactInteger(ref str);
            for (var i = 0; i < args.Value; i++)
            {
                var fca = new FunctionCallArgV7();
        ...
        private StorageV7 GetStorageV7(ref string str)
        {
            var storage = new StorageV7();
        ...
        private ConstV7 GetConstV7(ref string str)
        {
            var consts = new ConstV7();
